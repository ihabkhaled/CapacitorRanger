...
- **Errors.** Announced, associated with their field, and phrased as a fix. `form-field.component.tsx`
  owns the association; the hook owns the copy.
- **Live regions.** Async outcomes — a toast, the offline banner, a retry result — reach a screen
  reader instead of only the eye.
- **RTL, not just translation.** Arabic flips direction. Direction is derived centrally from locale
  and applied to the document root; check logical properties over `left`/`right`, and mirrored
  chevrons and animations.
...
- Is the accessible name the translated string, or the key?

## Commands it runs

```bash
npm run test:a11y       # axe over welcome, login (+error), settings, workbench, home,
                        # the dark palette, the Arabic RTL layout, and target size
...
- **[19-accessibility](../rules/19-accessibility.md)** and **[21-i18n-rtl](../rules/21-i18n-rtl.md)**
  are normative; this lens applies them and looks past them.
...
- **`architecture/no-raw-i18n-text`** already rejects hardcoded JSX text; do not re-report what lint
  reports.
- **The testing reviewer** for coverage of the new states, and the **performance reviewer** for
  animation and rendering concerns.
- **The source and the axe output** over any assertion in this file.
