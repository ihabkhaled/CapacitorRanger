...
the version already installed (`$eslint` = the root `eslint` entry, currently `10.8.0`), rather than
...
| `typescript`                       | `6.0.2`   | Parser-only; `typescript-eslint@8.65.0` peers `>=4.8.4 <6.1.0`                       |
| `@typescript/native`               | → `7.0.2` | The primary compiler; aliased because two majors cannot share a name                 |
...
- `@ionic/react` + `@ionic/react-router` — same version line (`8.8.15`).
...
- `typescript` (parser) is **not** coupled to `@typescript/native` (primary) — they move independently.
